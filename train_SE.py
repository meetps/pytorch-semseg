...
def train(cfg, writer, logger):
    ...
    params = sum([np.prod(p.size()) for p in filter(lambda p: p.requires_grad, model.parameters())])/1e6
    ...
    while i_train_iter < cfg['training']['train_iters']:
        ...
        for (images, labels, case_index_list) in trainloader:
            ...
            loss = cfg['training']['loss_balance_ratio'] * loss_fn(input=outputs_FM, target=labels, weight=weight, size_average=cfg['training']['loss']['size_average']) #Input:FM, Softmax is built with crossentropy loss fucntion
        ...
        if not validation_check:
            ...
            with torch.no_grad():
                for i_val, (images_val, labels_val, case_index_list_val) in enumerate(valloader):
                    ...
                    val_loss= cfg['training']['loss_balance_ratio'] * loss_fn(input=outputs_FM_val, target=labels_val, weight=weight, size_average=cfg['training']['loss']['size_average'])
            ...
            logger.info("Iter %d Loss_total: %.4f" % (i_train_iter + 1, val_loss_meter.avg))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="configs/linknet3D_SqueezeExcitation.yml",
